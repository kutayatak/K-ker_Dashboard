...
export function ExcelView() {
  ...
  const handleAddSave = async (formValues: typeof addForm) => {
    if (!addingTaskState) return;
    setEditSaving(true);
    try {
      const [hh, mm] = formValues.time.split(":").map(Number);
      ...
      if (isTechnical) {
        pickupLoc = formValues.hotelName.trim() || "Teknik İş";
        dropoffLoc = "Teknik İş";
      } else if (isExtra) {
        pickupLoc = formValues.hotelName.trim() || "Ekstra İş";
        dropoffLoc = isRight ? "Ekstra Gider" : "Ekstra Gelir";
      } else if (isRight) {
        pickupLoc = "Esenboğa Havalimanı";
        dropoffLoc = formValues.hotelName.trim() || "Otel";
      } else {
        pickupLoc = formValues.hotelName.trim() || "Otel";
        ...
        createTaskMutation.mutate(
          {
            data: {
              ...
              flightCode: formValues.flightCode.trim() || undefined,
              pickupLocation: pickupLoc,
              dropoffLocation: dropoffLoc,
              scheduledTime,
              passengerCount: 1,
              notes: formValues.notes.trim() || undefined,
              km: formValues.km === "" ? null : Number(formValues.km),
    ...
    <div className="flex flex-col h-full gap-4">
      ...
      {addingTaskState && (
        <AddTaskDialog
          ...
          onClose={() => setAddingTaskState(null)}
